...
(async () => {
  ...
  async function countHashesToTreasury() {
    // We will fetch recent signatures for treasury and count them.
    // Note: pagination may be needed for many txs; for hackathon limit to 1000.
    const sigs = await connection.getSignaturesForAddress(TREASURY, {limit: 1000});
    // Optionally filter memos/your payloads by fetching tx details. Simpler: count all transfers in that list.
    let count = 0;
    for (const s of sigs) {
      ...
      for (const ix of inner) {
        try {
          // If this instruction is a system transfer to the treasury
          if (ix.programId.equals(SystemProgram.programId)) {
            // decode accounts: instruction data layout varies; we instead inspect postBalances difference
            // simpler approach: treat every tx with treasury as a sign it's a payment for hashing (approx)
            if (tx.meta && tx.meta.postBalances) {
              // naive: if treasury appears in accountKeys
          ...
          // Also detect memo by program id
  ...
  window.hashArticleAndPay = async function(articleTextOrUrl) {
    try {
      ...
      console.log('✅ Sent tx', txid, payload, 'price (SOL):', nextPriceSol);
  ...
  window.fetchArticleText = async function(url) {
    ...
    // crude: strip tags. For production use a proper parser / server-side extraction.
